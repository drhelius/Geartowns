...
{
    runahead_audio = new s16[GT_AUDIO_BUFFER_SIZE];
...
void runahead_run(int frames, u8* frame_buffer, s16* sample_buffer, int* sample_count)
{
    ...
    core->RunToFrame(frame_buffer, sample_buffer, sample_count);
    ...
    for (int i = 0; i < frames; i++)
    {
        ...
        core->RunToFrame(frame_buffer, runahead_audio, &discarded_samples, render);
    }
