...
4. Post a clarification comment only when the issue is likely a `bug`, the next engineering action depends on Geartowns version or platform/frontend, and the issue body does not already provide that information.
5. When asking, ask only for Geartowns version and platform/frontend. Do not ask for ROM hashes, logs, screenshots, save files, long reproduction forms, or extra context during initial triage.
...
```markdown
Automated triage request: please provide the Geartowns version and platform/frontend so this bug report can be investigated.
