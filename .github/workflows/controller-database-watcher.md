...
Check whether Geartowns's `platforms/shared/gamecontrollerdb.txt` differs from the latest upstream SDL controller database and open a pull request when an update is available.
...
End the PR body with this exact sentence:

`This is an automated pull request generated by [Controller Database Watcher](https://github.com/${{ github.repository }}/actions/workflows/controller-database-watcher.lock.yml)`

## Style

Keep the PR body factual and neutral. Use only the linked automation sentence above as the sign-off. Do not use first person, apologies, thanks, conversational warmth, or human-like sign-offs.